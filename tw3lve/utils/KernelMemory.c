...
uint32_t rk32_via_kmem_read_port(uint64_t kaddr)
{
    ...
    if (kmem_read_port == MACH_PORT_NULL) {
        LOGME("kmem_read_port not set, have you called prepare_rk?");
        exit(EXIT_FAILURE);
    }
    ...
    if (err != KERN_SUCCESS) {
        LOGME("error setting context off of dangling port: %x %s", err, mach_error_string(err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err != KERN_SUCCESS) {
        LOGME("error calling pid_for_task %x %s", err, mach_error_string(err));
        exit(EXIT_FAILURE);
    }
...
uint64_t kmem_alloc_wired(uint64_t size)
{
    ...
    LOGME("vm_kernel_page_size: %lx", vm_kernel_page_size);
    ...
    LOGME("allocated address: %llx", addr);
    
    addr += 0x3fff;
    addr &= ~0x3fffull;
    
    LOGME("address to wire: %llx", addr);
    
    err = mach_vm_wire(fake_host_priv(), tfp0, addr, ksize, VM_PROT_READ | VM_PROT_WRITE);
    if (err != KERN_SUCCESS) {
        LOGME("unable to wire kernel memory via tfp0: %s %x", mach_error_string(err), err);
        return 0;
    }
    return addr;
}
...
{
    if (tfp0 == MACH_PORT_NULL) {
        LOGME("attempt to change protection of kernel memory before any kernel memory write primitives available");
        return false;
    }
    kern_return_t err;
    err = mach_vm_protect(tfp0, (mach_vm_address_t)kaddr, (mach_vm_size_t)size, 0, (vm_prot_t)prot);
    if (err != KERN_SUCCESS) {
        LOGME("unable to change protection of kernel memory via tfp0: %s %x", mach_error_string(err), err);
        return false;
    }
    return true;
}
