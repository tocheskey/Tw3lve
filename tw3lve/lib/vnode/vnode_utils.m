...
uint64_t zm_fix_addr(uint64_t addr) {
    ...
    if (zm_hdr.start == 0) {
        ...
        size_t r = kread2(zone_map + 0x10, &zm_hdr, sizeof(zm_hdr));
...
uint64_t vnodeForPath(const char *path) {
    ...
    if (!ISADDR(vfs_context)) {
        LOGME("Failed to get vfs_context.");
        goto out;
    }
    vpp = malloc(sizeof(uint64_t));
    if (vpp == NULL) {
        LOGME("Failed to allocate memory.");
        goto out;
    }
    if (_vnode_lookup(path, O_RDONLY, vpp, vfs_context) != ERR_SUCCESS) {
        LOGME("Failed to get vnode at path \"%s\".", path);
        goto out;
    }
    vnode = *vpp;
    out:
...
uint64_t vnodeForSnapshot(int fd, char *name) {
    ...
    LOGME("rvpp_ptr = " ADDR, rvpp_ptr);
    if (!ISADDR(rvpp_ptr)) {
        goto out;
    }
    sdvpp_ptr = kmem_alloc(sizeof(uint64_t));
    LOGME("sdvpp_ptr = " ADDR, sdvpp_ptr);
    if (!ISADDR(sdvpp_ptr)) {
        goto out;
    }
    ndp_buf = kmem_alloc(816);
    LOGME("ndp_buf = " ADDR, ndp_buf);
    if (!ISADDR(ndp_buf)) {
        goto out;
    }
    vfs_context = _vfs_context();
    LOGME("vfs_context = " ADDR, vfs_context);
    if (!ISADDR(vfs_context)) {
        goto out;
    }
    ...
    LOGME("snap_meta = " ADDR, snap_meta);
    if (!ISADDR(snap_meta)) {
        goto out;
    }
    ...
    LOGME("snap_vnode = " ADDR, snap_vnode);
