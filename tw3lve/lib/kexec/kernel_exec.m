...
static mach_port_t prepare_user_client()
{
    ...
    if (service == IO_OBJECT_NULL) {
        LOGME("unable to find service");
        exit(EXIT_FAILURE);
    }
    
    err = IOServiceOpen(service, mach_task_self(), 0, &user_client);
    if (err != KERN_SUCCESS) {
        LOGME("unable to get user client connection");
        exit(EXIT_FAILURE);
    }
    
    LOGME("got user client: 0x%x", user_client);
    return user_client;
}
...
uint64_t kexecute2(uint64_t addr, uint64_t x0, uint64_t x1, uint64_t x2, uint64_t x3, uint64_t x4, uint64_t x5, uint64_t x6)
{
    ...
    LOGME(""ADDR"("ADDR", "ADDR", "ADDR", "ADDR", "ADDR", "ADDR", "ADDR"): "ADDR"", addr, x0, x1, x2, x3, x4, x5, x6, returnval);
    return returnval;
}
